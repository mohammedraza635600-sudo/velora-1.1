...
export default function ProductCard({p}:{p:Product}){
  ...
  return <article className="group">
    <div className="relative aspect-[3/4] overflow-hidden bg-soft/40">
      ...
      <button onClick={()=>toggleWish(p.id)} aria-pressed={liked} aria-label="Wishlist" className={`absolute top-3 right-3 text-xs tracking-widest uppercase px-3 py-1.5 bg-ivory/90 ${liked?'text-olive':''}`}>{liked?'Saved':'Save'}</button>
      <div className="absolute inset-x-0 bottom-0 flex translate-y-full group-hover:translate-y-0 focus-within:translate-y-0 transition duration-500 text-xs tracking-[.15em] uppercase">
    ...
    <div className="mt-4 flex justify-between gap-4"><div><h3 className="text-sm">{p.name}</h3><p className="text-xs text-sage capitalize">{p.category.replace('-',' ')} · {p.color}</p></div><p className="text-sm">{p.discount_price?<><s className="text-sage mr-2">{inr(p.price)}</s>{inr(p.discount_price)}</>:inr(p.price)}</p></div></article>}
