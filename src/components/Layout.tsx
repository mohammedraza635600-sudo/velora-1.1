  ...
  <header className={`fixed top-0 inset-x-0 z-50 transition-all duration-500 ${solid?'bg-ivory/85 backdrop-blur-md border-b border-soft/60 text-forest':'text-ivory'}`}>
    <div className="flex items-center justify-between px-5 md:px-10 h-16 md:h-20">
      ...
      <div className="flex items-center gap-5 uppercase">
        <button onClick={()=>nv('/shop?focus=1')} className="hdr-util">Search</button>
        <Link to="/account" className="hdr-util hidden md:block">Account</Link>
        <Link to="/wishlist" className="hdr-util hidden md:block">Wishlist{wish.length?` (${wish.length})`:''}</Link>
        <Link to="/cart" className="hdr-util">Cart ({count})</Link>
  ...
  <footer className="bg-forest text-ivory mt-32 px-5 md:px-10 pt-20 pb-10">
    <div className="grid md:grid-cols-4 gap-12"><div><p className="font-serif text-4xl tracking-[.3em]">VELORA</p><p className="mt-4 text-soft max-w-xs">Menswear designed for a refined everyday wardrobe.</p></div>
      ...
      <form onSubmit={async e=>{e.preventDefault();const f=e.currentTarget,em=String(new FormData(f).get('email'));alert(await subscribe(em)?'Thank you. You are on the list.':'Could not sign you up. Try again.');f.reset()}}>
        ...
        <button className="btn bg-ivory text-forest border-ivory hover:bg-soft mt-4">Subscribe</button></form></div>
    <div className="mt-8 pt-6 border-t border-olive/60 text-sm text-soft flex flex-wrap gap-x-8 gap-y-2">{ft.email&&<a href={'mailto:'+ft.email}>{ft.email}</a>}{ft.phone&&<a href={'tel:'+ft.phone}>{ft.phone}</a>}{lines(ft.socials).map(x=><L key={x} v={x}/>)}{lines(ft.policies).map(x=><L key={x} v={x}/>)}</div>
    <p className="mt-3 text-xs text-sage">{ft.copyright||`© ${new Date().getFullYear()} VELORA. All rights reserved.`}</p></footer>
